...
let net_record rpc session_id net =
  ...
  { setref=(fun r -> _ref := r; record := empty_record );
    ...
    fields = [
      ...
      make_field ~name:"is-default-bridge" ~get:(fun () -> string_of_bool (x ()).API.network_is_default_bridge)  
        ~set:(fun x -> Client.Network.set_is_default_bridge rpc session_id net (safe_bool_of_string "is-default-bridge" x)) ();
