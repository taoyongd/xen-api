...
let initialise session_id template pool =
  ...
  let networks = Array.init networks_to_create (fun i ->
      Client.Network.create ~rpc ~session_id ~name_label:(Printf.sprintf "perftestnet%d" i) ~name_description:"" ~mTU:1500L ~other_config:[oc_key,pool.key] ~is_default_bridge:false  ~tags:[])
  in
...
let create_sdk_pool session_id sdkname pool_name key ipbase =
  ...
  let bondednets = Array.init pool.bonds (fun i ->
      Client.Network.create poolrpc poolses (Printf.sprintf "Network associated with bond%d" i) "" 1500L false [] [])
  in
